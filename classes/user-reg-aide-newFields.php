	...
	function csds_userRegAide_editNewFields(){
		...
		if (isset($_POST['delete_field'])){
			...
			if(current_user_can('activate_plugins')){
				...
				//$ura->csds_delete_field_from_users_meta($results1); 
		...
		echo '<div id="wpbody">';
			echo '<div class="wrap">';
			do_action('create_tabs', 'edit_new_fields'); // Line 255 user-registration-aide.php
			if(!empty($msg)){
				echo $msg;
			} ?>
				<form method="post" name="csds_userRegAide_newFields" id="csds_userRegAide_newFields"> 
				<h2 class="adminPage"><?php _e('User Registration Aide: Edit New Fields', 'csds_userRegAide');?></h2>
					<div id="poststuff">
						<div class="stuffbox"><span class="regForm"><?php _e('Delete or Change Field Order or Change Field Display Title for New Fields Here:', 'csds_userRegAide');?> </span>
						<?php // Delete new fields form ?>
							<div class="inside">
							<table class="editNewFields">
							<tr>
							...
							echo '<br/>';
							echo '<table class="newFields1">';
							echo '<tr>';
							echo '<th>'. __('Additional New Field Name: ', 'csds_userRegAide').'</th>';
							echo '<th>'. __('Current Field Order: ', 'csds_userRegAide').'</th>';
							echo '</tr>';
							if(!empty($csds_userRegAide_fieldOrder)){
								...
								echo '<tr>';
								echo '<td class="fieldName" colspan="2">';
								echo '<p class="deleteFields">';
								_e('No new fields currently exist, you have to add new fields on the main page before you can change the order!', 'csds_userRegAide');
								echo '</p>';
								echo '</td>';
								echo '</tr>';
								echo '</table>';
							...
							echo '<td>';
							$newFields = get_option('csds_userRegAide_NewFields');
							echo '<p>'.__('Here you can change the field name displayed to the user for the new additional fields on the registration form and profile. !', 'csds_userRegAide') .'</p>';
							$i = '';
							$cnt = '';
							$fieldKey = '';
							$fieldKeyUpper = '';
							...
							echo '<br/>';
							echo '<table class="newFields2">';
							echo '<tr>';
							echo '<th>'. __('Additional New Field ID: ', 'csds_userRegAide').'</th>';
							echo '<th>'. __('Current Field Title: ', 'csds_userRegAide').'</th>';
							echo '</tr>';
							if(!empty($newFields)){
								foreach($newFields as $fieldKey => $fieldName){ ?>
									<tr>
